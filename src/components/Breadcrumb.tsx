  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Top Trail Navigation & Back Action Bar */}
        <div className="flex flex-wrap items-center justify-between gap-3 mb-8">
          {/* Breadcrumb Trail Navigation */}
          <nav aria-label="Breadcrumb Navigation">
            <ol className="inline-flex items-center flex-wrap gap-1.5 sm:gap-2 text-xs bg-slate-900/80 backdrop-blur-md border border-slate-800/90 px-3.5 py-1.5 rounded-xl text-slate-300 shadow-inner">
              {/* Home Link */}
              <li>
                <button
          ...
          {backAction && (
            <button
              onClick={backAction}
              className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-slate-900/80 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-800/90 transition-all cursor-pointer text-xs font-semibold shadow-xs backdrop-blur-md"
            >
        ...
        {/* Hero Header Content */}
        <div className={align === 'center' ? 'text-center max-w-4xl mx-auto space-y-4' : 'space-y-4 max-w-4xl'}>
          {/* Eyebrow badge */}
          <div className={`inline-flex items-center gap-2 text-indigo-300 text-xs font-mono font-bold tracking-[0.2em] uppercase ${align === 'center' ? 'justify-center' : ''}`}>
            <span className="w-2 h-2 rounded-full bg-indigo-400 animate-pulse"></span>
            <span>{pageBadge}</span>
          </div>

          {/* Prominent Page Title */}
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white tracking-tight font-['Outfit'] leading-tight">
            {pageTitle}
          </h1>

          {/* Subtitle */}
          {subtitle && (
            <p className={`text-xs sm:text-sm md:text-base text-slate-300 max-w-3xl leading-relaxed ${align === 'center' ? 'mx-auto' : ''}`}>
              {subtitle}
            </p>
          )}
        </div>
